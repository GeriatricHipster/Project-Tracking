...
export function buildBlankOwnerCmsGrid(rowCount = ROW_COUNT) {
  return Array.from({ length: Math.max(ROW_COUNT, rowCount) }, () => Array.from({ length: COLUMN_COUNT }, () => ''));
}

export function normalizeOwnerCmsGrid(cells, minRows = ROW_COUNT) {
  const safeRows = Array.isArray(cells) ? cells.length : 0;
  const grid = buildBlankOwnerCmsGrid(Math.max(minRows, safeRows));
